...
function renderKpis(d, s) {
  ...
  if (d.municipios_comparables && d.municipios_comparables > 0) {
    const pct = d.regresion_pct ?? 0;
    regrCls = pct >= 50 ? 'kpi--down' : (pct > 0 ? '' : 'kpi--up');
    regrValue = `${pct.toFixed(1)}%`;
  ...
  if (s.municipio) {
    if (d.delta == null) {
      regrValue = '—';
      regrHint  = 'sin dato del año previo';
      regrCls   = '';
    } else if (d.delta < 0) {
      regrValue = '↓ Pierde';
      regrHint  = formatCompact(d.delta) + ' hab. vs ' + (s.anio - 1);
      regrCls   = 'kpi--down';
    } else {
      regrValue = '↑ Crece';
      regrHint  = '+' + formatCompact(d.delta) + ' hab. vs ' + (s.anio - 1);
