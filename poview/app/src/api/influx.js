...
export function fetchMunicipios(provincia) {
  if (!provincia) return Promise.resolve([]);
  return getJson('/municipios', { provincia }).catch(err => {
    console.error('[influx] fetchMunicipios falló:', err.message);
    return [];
  });
}

export function fetchValoresMapa({ anio, sexo, comunidad, provincia } = {}) {
  return getJson('/mapa', { anio, sexo, comunidad, provincia })
    .then(rows => {
      const map = {};
